  ...
  return (
    <>
      ...
      <div className={`fixed bottom-24 right-4 sm:right-8 w-[calc(100%-2rem)] max-w-sm h-[70vh] max-h-[500px] bg-white dark:bg-gray-800 rounded-lg shadow-2xl flex flex-col transition-all duration-300 ease-in-out z-40 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}>
        <header className="flex items-center justify-between p-4 border-b dark:border-gray-700">
          <h3 className="text-lg font-bold flex items-center gap-2">
            <Bot className="text-indigo-500"/> CareerBot
          </h3>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
              ...
              <div className={`max-w-xs md:max-w-md lg:max-w-lg rounded-lg px-4 py-2 ${msg.sender === 'bot' ? 'bg-gray-100 dark:bg-gray-700' : 'bg-indigo-500 text-white'}`}>
            ...
            <div className="pt-2">
              <div className="flex flex-col items-start gap-2">
                {predefinedQuestions.map((q, i) => (
                  <button key={i} onClick={() => handleQuestionClick(q)} className="w-full text-left text-sm text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-gray-700 hover:bg-indigo-100 dark:hover:bg-gray-600 p-2 rounded-md transition-colors">
                    {q}
                  </button>
                ))}
              </div>
        ...
        <footer className="p-4 border-t dark:border-gray-700">
            <form onSubmit={handleFormSubmit} className="flex items-center gap-2">
                <input
                    ...
                    disabled={isLoading}
                    className="flex-1 p-2 border rounded-md bg-gray-50 dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:outline-none disabled:opacity-50"
                    aria-label="Your message"
                />
                <button
                    type="submit"
                    disabled={isLoading || !inputValue.trim()}
                    className="bg-indigo-600 text-white p-2 rounded-md hover:bg-indigo-700 transition-colors disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed"
      ...
      <button
        ...
        className="fixed bottom-6 right-4 sm:right-8 bg-indigo-600 text-white w-14 h-14 rounded-full shadow-lg flex items-center justify-center hover:bg-indigo-700 transition-transform duration-200 hover:scale-110 z-50"
